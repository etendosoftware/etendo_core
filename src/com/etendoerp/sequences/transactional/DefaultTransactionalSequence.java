...
public class DefaultTransactionalSequence extends DefaultSequenceGenerator {
    ...
    public String generateValue(Session session, Object owner) {
        ...
        SequenceParameterList defaultSequenceParameterList = SequenceParametersUtils.generateDefaultParameterList(baseOBObject);
        defaultSequenceParameterList.setParameter(SequenceDatabaseUtils.PROPERTY_COLUMN, SequenceParametersUtils.generateTableDirParameter(columnId));

        SequenceParameterList sequenceParameterList = SequenceParametersUtils.getParametersFromBaseOBObject(baseOBObject, sequenceDimensionList);
        ...
        try {
            ...
            log.error("Error generating the next sequence value for the field {} " +
                        ". No Sequence found using the parameters {}", column.getDBColumnName(), sequenceParameterList.getParameterValues());
            String message = OBMessageUtils.getI18NMessage(SequenceDatabaseUtils.SEQUENCE_NOT_FOUND) + " " + property.getNameOfColumn();
