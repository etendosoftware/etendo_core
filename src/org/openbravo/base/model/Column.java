...
public class Column extends ModelObject {
  ...
  public boolean isSequence() {
    return (referenceValue != null && !referenceValue.getSequenceConfigurations().isEmpty()) || !reference.getSequenceConfigurations().isEmpty();
  }

  public String getDBSequenceName() {
    var config = getSequenceConfiguration();
    String name = null;

    if (config.isPresent()) {
      name = config.get().getDbSequenceName();
    }
  ...
  public String getDBSequenceInitialValue() {
    ...
    if (config.isPresent()) {
      initialValue = config.get().getDbSequenceInitial();
    }
  ...
  public String getDBSequenceIncrementValue() {
    ...
    if (config.isPresent()) {
      incrementValue = config.get().getDbSequenceIncrement();
    }
  ...
  public String getSequenceGeneratorClassName() {
    ...
    if (isSequence()) {
      className = getGeneratorClassFromReference(referenceValue);

      if (className == null) {
        // reference value has no generator declared, use the base one
        className = getGeneratorClassFromReference(reference);
      }
    }
  ...
  public String getGeneratorClassFromReference(Reference reference) {
    if (reference == null) {
      return null;
    }
    var sequenceConfigs = reference.getSequenceConfigurations();
    for (SequenceConfiguration config : sequenceConfigs) {
      String generatorJavaClass = config.getGenerator();
      if (!StringUtils.isBlank(generatorJavaClass)) {
        return generatorJavaClass;
      }
    }
    return null;
  }

  public Optional<SequenceConfiguration> getSequenceConfiguration() {
    Optional<SequenceConfiguration> config = Optional.empty();
    if (referenceValue != null) {
      config = referenceValue.getSequenceConfigurations().stream().findFirst();
    }
    if (config.isEmpty()) {
      config = reference.getSequenceConfigurations().stream().findFirst();
    }
