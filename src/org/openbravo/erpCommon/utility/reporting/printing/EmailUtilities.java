...
public class EmailUtilities {
    ...
    public static void prepareAndSendEmail(Report report, VariablesSecureApp vars,
        ...
        toEmail = getEmailValue(moreThanOneCustomer, documentData.contactEmail,  vars.getStringParameter("toEmail"), true, "NoSalesRepEmail");
        replyToEmail = getEmailValue(moreThanOnesalesRep, documentData.salesrepEmail,  vars.getStringParameter("replyToEmail"), true, "NoCustomerEmail");
    ...
    private static Collection<? extends File> getDocumentAttachments(String documentId) throws IOException {
        ...
        try {
            ...
            attachmentOBCriteria.addEqual(Attachment.PROPERTY_RECORD, documentId);
            
            // Do not include reports generated by emailing using a NOT EXISTS subquery
            attachmentOBCriteria.addFunction((cb, obc) -> {
                Subquery<String> subquery = obc.cq.subquery(String.class);
                var metadataRoot = subquery.from(ParameterValue.class);
                subquery.select(metadataRoot.get("id"));
                subquery.where(
                    cb.and(
                        cb.equal(metadataRoot.get(ParameterValue.PROPERTY_FILE).get("id"), obc.root.get("id")),
                        cb.equal(metadataRoot.get(ParameterValue.PROPERTY_VALUESTRING), ReportManager.GENERATED_BY_EMAILING)
                    )
                );
    ...
    public static void sendEmail(EmailServerConfiguration mailConfig, EmailInfo email, List<File> attachments) throws ServletException, IOException {
        try {
    ...
    /**
     * @author gmauleon
     * @throws ServletException when loading the attachment from disk fails
     */
    ...
    public static String getEmailValue(boolean condition, String defaultValue, String alternativeValue, boolean failIfEmpty, String failureMessage) throws ServletException {
        String result;
        if (condition) {
            result = defaultValue;
        } else {
    ...
    public static PocData[] getContactDetails(DocumentType documentType, String strDocumentId, ConnectionProvider connectionProvider)
