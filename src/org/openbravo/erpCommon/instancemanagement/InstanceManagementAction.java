...
public class InstanceManagementAction extends Action {
    ...
    try {
      ...
      switch (command) {
        case "ACTIVATE":
          ...
          break;
        case "DEACTIVATE":
          result = handleDeactivate(parameters);
          break;
        case "CANCEL":
          result = handleCancel(parameters);
          break;
        case "INSTALLFILE":
  ...
  private ActionResult handleActivate(JSONObject parameters) {
    ...
    try {
      ...
      if ("Success".equals(processResult.getType())) {
        result.setType(Result.Type.SUCCESS);
   ...
   */
  private ActionResult handleDeactivate(JSONObject parameters) {
    ActionResult result = new ActionResult();
    ResponseActionsBuilder responseActions = getResponseBuilder();
   ...
   */
  private ActionResult handleCancel(JSONObject parameters) {
    ActionResult result = new ActionResult();
    ResponseActionsBuilder responseActions = getResponseBuilder();
   ...
   */
  private OBError activateCancelRemote(String publicKey, String purpose, String instanceNo,
      boolean activate) {
    try {
      VariablesSecureApp vars = RequestContext.get().getVariablesSecureApp();
      ProcessBundle pb = new ProcessBundle(null, vars);
