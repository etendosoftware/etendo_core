...
public class OBRestUtils {
  ...
  static public void addEntitynameToParams(String path, Map<String, String> parameters) {
    ...
    try {
      final TableAccess tableAccess = getTableAccess(entity);
      if (null == tableAccess) {
        throw new OBException(OBMessageUtils.messageBD("SMFSWS_TableAccessDenied"));
      }
    } catch (Exception e) {
  ...
  private static TableAccess getTableAccess(Entity entity) {
    final Role userRole = OBContext.getOBContext().getRole();
    OBContext.setAdminMode(true);
    final OBCriteria<TableAccess> tableAccessOBCriteria = OBDal.getInstance().createCriteria(TableAccess.class);
    tableAccessOBCriteria.add(Restrictions.eq(TableAccess.PROPERTY_ROLE, userRole));
    tableAccessOBCriteria.createAlias(TableAccess.PROPERTY_TABLE, "t", JoinType.INNER_JOIN);
    tableAccessOBCriteria.add(Restrictions.eq("t." + Table.PROPERTY_NAME, entity.toString()));
    final TableAccess tableAccess = (TableAccess) tableAccessOBCriteria.setMaxResults(1).uniqueResult();
  ...
  static public Map<String, String> mapRestParameters(Map<String, String> params) {
    ...
    // include identifiers
    parameters.put(OBRestConstants.IDENTIFIERS_PARAMETER,
        Boolean.toString("true".equals(params.get(OBRestConstants.IDENTIFIERS_PARAMETER))));
