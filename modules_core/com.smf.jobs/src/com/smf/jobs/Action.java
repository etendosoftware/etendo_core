...
public abstract class Action extends BaseProcessActionHandler {
     ...
     * @param parameters Process parameters defined in AD
     * @param content JSON with window and parameter data from the UI
    ...
    protected JSONObject doExecute(Map<String, Object> parameters, String content) {
        try {
            ...
            var allParams = jsonContent.has("_params") ? jsonContent.getJSONObject("_params") : new JSONObject();
            if (jsonContent.has("_buttonValue")) {
                allParams.put("_buttonValue", jsonContent.get("_buttonValue"));
            }

            var result = action(jsonContent.optJSONObject("_params"), new MutableBoolean(false));

            result = postRun(result);

            ActionResult finalResult = result;

            return result.getResponseActionsBuilder().map(b -> finalResult.toJSON(b, false)).orElse(finalResult.toJSON(getResponseBuilder(), true));
     ...
     * @param jsonContent json content will not be null when coming from the UI (Process Definition)
     ...
     * @param result the ActionResult of an execution
     ...
     * @param input The input data that will come from a Filter
     * @param stopped If the action was stopped due to a kill signal on the Job that runs it.
     ...
     * @param parameters JSONObject with parameter-value map.
     */
     ...
     * @param entityClass the class representing an entity (example: Invoice.class)
     * @param <T> the type of the entityClass parameter. Must extend BaseOBObject
     ...
     */
    protected Data getInput() { return input; }

    /**
     * Sets the action input.
     * @param input {@link Data} with new contents.
     */
     ...
     */
    protected Map<String, Object> getRequestParameters() { return requestParameters; }

    /**
     * Sets the action requestParameters.
     * @param requestParameters Process parameters defined in AD
     */
     ...
     * @param parameters Parameters (if any) defined in dictionary
     * @param isStopped true when the Job that runs the action was signaled to be stopped.
