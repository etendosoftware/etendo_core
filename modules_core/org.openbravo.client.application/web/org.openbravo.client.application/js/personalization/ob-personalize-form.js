...
isc.OBPersonalizeFormLayout.addProperties({
  ...
  createAddMainLayout: function() {
    var me = this,
      fieldsTabSet,
      leftLayout,
      mainLayout = isc.VLayout.create({}, OB.Styles.Personalization.MainLayout);
  ...
  createPropertiesLayout: function() {
    var NumericField,
      CheckboxField,
      propertiesLayout = isc.Layout.create(
    ...
    propertiesLayout.emptyMessage = isc.Layout.create({
      ...
      members: [
        isc.Label.create({
          ...
          height: 1,
          overflow: 'visible',
          contents: OB.I18N.getLabel(
            'OBUIAPP_Personalization_PropertiesFormEmptyMessage'
          )
        })
      ]
    });
    ...
    this.propertiesTabSet = isc.OBTabSet.create(
      OB.Styles.Personalization.TabSet,
      {
        ...
        toggleVisualState: function() {
          if (this.expanded) {
            this.setHeight(
              OB.Styles.Personalization.PropertiesTabSet.collapsedHeight
            );
            this.expanded = false;
          } else {
            this.setHeight(
              OB.Styles.Personalization.PropertiesTabSet.expandedHeight
            );
            this.expanded = true;
          }
        },
    ...
    propertiesLayout.updatePropertiesDisplay = function(record) {
      if (!record) {
        ...
        this.propertiesTabSet.setTabTitle(
          this.propertiesTabSet.getTab(0),
          OB.I18N.getLabel('OBUIAPP_Personalization_HeaderProperties')
        );
      } else {
        this.propertiesTabSet.setTabTitle(
          this.propertiesTabSet.getTab(0),
  ...
  createAddToolbar: function() {
    ...
    saveCloseButtonProperties = {
      ...
      updateState: function() {
        ...
        if (this.saveDisabled) {
          ...
          this.prompt = OB.I18N.getLabel(
            'OBUIAPP_Personalization_Statusbar_Close'
          );
        } else {
          this.buttonType = 'saveclose';
          this.prompt = OB.I18N.getLabel(
            'OBUIAPP_Personalization_Toolbar_SaveClose'
          );
        }
        this.resetBaseStyle();
      },
    ...
    deleteButtonProperties = {
      ...
      updateState: function() {
        ...
        this.setDisabled(
          this.view.openedFromMaintenanceWindow ||
            (this.view.form &&
    ...
    restoreButtonProperties = {
      action: function() {
        var i,
          standardWindow = this.view.getStandardWindow(),
    ...
    this.toolBar = isc.OBToolbar.create({
      view: this,
      leftMembers: [
        isc.OBToolbarIconButton.create(saveButtonProperties),
  ...
  save: function(callback) {
    var params,
      me = this,
      newDataFields,
      formPers = this.openedFromMaintenanceWindow
        ? this.personalizationData
        : this.form.view.getFormPersonalization();
    ...
    OB.RemoteCallManager.call(
      ...
      function(resp, data, req) {
        ...
        if (!me.openedFromMaintenanceWindow) {
          ...
          me.form.view.standardWindow.updateFormPersonalization(
            me.form.view,
            me.personalizationData
          );
        }
  ...
  deletePersonalization: function(confirmed) {
    ...
    if (!confirmed) {
      ...
      isc.ask(
        OB.I18N.getLabel('OBUIAPP_Personalization_ConfirmDelete'),
        callback
      );
      return;
    }
    ...
    OB.RemoteCallManager.call(
      ...
      function(resp, data, req) {
        ...
        if (me.getStandardWindow().getClass().personalization.forms) {
          me.getStandardWindow().getClass().personalization.forms[
            me.tabId
          ] = null;
        }
      }
  ...
  cancel: function(confirmed) {
    ...
    if (!confirmed) {
      ...
      isc.ask(
        OB.I18N.getLabel('OBUIAPP_Personalization_ConfirmCancel'),
        callback
      );
      return;
    }
  ...
  buildPreviewForm: function() {
    ...
    this.previewForm = isc.OBViewForm.create(
      isc.clone(OB.ViewFormProperties),
      this.previewFormProperties,
      {
  ...
  buildFormAndTree: function() {
    ...
    if (!this.personalizationData || !this.personalizationData.form) {
      if (this.form) {
        computedPersonalizationData = OB.Personalization.getPersonalizationDataFromForm(
          this.form
        );
      } else {
    ...
    OB.Personalization.updatePersonalizationDataFromFields(
      this.personalizationData.form.fields,
      this.previewForm.getFields(),
      this.previewForm.statusBarFields || []
    );
  ...
  doClose: function(confirmed) {
    ...
    if (this.isChanged && !confirmed) {
      ...
      isc.ask(
        OB.I18N.getLabel('OBUIAPP_Personalization_ConfirmClose'),
        callback
      );
      return;
    }
  ...
  doOpen: function(retrievedInitialData) {
    ...
    if (!retrievedInitialData) {
      OB.RemoteCallManager.call(
        ...
        function(resp, data, req) {
          ...
          // copy some stuff
          me.previewFormProperties._originalFields = isc.clone(
            me.previewFormProperties.fields
          );
    ...
    tabSet.setTabTitle(
      tab,
      OB.I18N.getLabel('OBUIAPP_Personalize_TitlePrefix', [this.tabIdentifier])
    );
    delete this.initializing;
  },
  ...
  setStatusBarInformation: function() {
    ...
    this.statusBar.setContentLabel(
      icon,
      label,
      statusBarFields,
      OB.I18N.getLabel('OBUIAPP_WindowPersonalization_Guidance')
    );
  }
});
