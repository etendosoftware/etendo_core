...
public class UserDefaultForDocsEventHandlerTest {
  ...
  /**
   * When {@code isDefaultfordocs} is {@code false} the handler must return early without
  ...
  /**
   * When the user has no associated Business Partner the handler must return early without
   * querying the database.
   */
  @Test
  ...
  /**
   * When the user is marked as default for docs, has a Business Partner, and no other user in
   * that Business Partner holds the flag, no exception must be thrown.
   */
  @Test
  ...
  /**
   * When the user is marked as default for docs and another user in the same Business Partner
   * already holds that flag, an {@link OBException} must be thrown.
   */
  @Test
  public void testValidateThrowsWhenConflictExists() {
    ...
    try (MockedStatic<ModelProvider> mpMock = mockStatic(ModelProvider.class);
      ...
      when(criteria.uniqueResult()).thenReturn(mock(User.class));
      msgMock.when(() -> OBMessageUtils.messageBD(anyString())).thenReturn(CONFLICT_ERROR);
  ...
  /**
   * Saving a new {@link User} that is the first default for docs in its Business Partner must not
   * throw any exception.
   */
  @Test
  public void testOnSaveDoesNotThrowWhenNoConflict() {
    UserDefaultForDocsEventHandler handler = new UserDefaultForDocsEventHandler() {
      ...
      protected boolean isValidEvent(
          org.openbravo.client.kernel.event.EntityPersistenceEvent event) {
        return true;
      }
    };
    try (MockedStatic<ModelProvider> mpMock = mockStatic(ModelProvider.class);
  ...
  /**
   * Saving a new {@link User} as default for docs when another user in the same Business Partner
   * already holds that flag must throw an {@link OBException}.
   */
  @Test
  ...
  /**
   * Updating a {@link User} to be the default for docs when no other user in the same Business
   * Partner holds that flag must not throw any exception.
   */
  @Test
  ...
  /**
   * Updating a {@link User} as default for docs when another user in the same Business Partner
   * already holds that flag must throw an {@link OBException}.
   */
  @Test
  ...
  /**
   * {@code getObservedEntities()} must return a non-null array so the CDI framework can register
