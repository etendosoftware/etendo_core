...
public class CancelAndReplaceTestData17 extends CancelAndReplaceTestData {
  ...
  public void initialize() {
    setTestNumber("CANCELREPLACE017");
    setTestDescription(
        "Cancel and Multiple Replace of a fully paid Order. Decrease quantity of a line. Netting shipment is created");
    ...
    setNewOrders(Arrays.asList(new CancelAndReplaceOrderTestData().with(newOrder -> {
      ...
      newOrder.status = "CO";
      newOrder.paidAmount = new BigDecimal("40.70");
      ...
      newOrder.status = "CO";
      newOrder.paidAmount = new BigDecimal("253.00");
      newOrder.outstandingAmount = new BigDecimal("-247.50");
      newOrder.lines = new Line[] { newOrder.new Line().with(line -> {
        line.deliveredQty = BigDecimal.ZERO;
        line.shipmentLines = BigDecimal.ZERO;
        line.orderedQuantity = BigDecimal.ONE;
