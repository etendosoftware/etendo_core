...
public final class ProductServlet extends HttpServlet {
    ...
    protected void service(HttpServletRequest request, HttpServletResponse response) throws java.io.IOException {
        ...
        try {
            ...
            String[] identity = authenticate(request.getHeader("Authorization"),
                    request.getHeader("X-Platform-Role"), request.getHeader("X-Platform-Organization"));
            if (identity == null) {
            ...
            OBContext.getOBContext().getEntityAccessChecker().checkReadableAccess(entity);
            response.setHeader("X-Platform-Role", identity[1]);
            response.setHeader("X-Platform-Client", identity[2]);
            response.setHeader("X-Platform-Organization", identity[3]);
    ...
    private String[] authenticate(String authorization, String requestedRole, String requestedOrganization) {
        ...
        for (String id : new String[] {requestedRole, requestedOrganization}) {
            if (id != null && !id.matches("[A-Za-z0-9]{1,32}")) throw new IllegalArgumentException("Invalid context ID");
        }
        ...
        try (var session = SessionFactoryController.getInstance().getSessionFactory().openSession()) {
            session.doWork(connection -> {
                try (var statement = connection.prepareStatement(
                        "select ad_user_id,password from ad_user where username=? and isactive='Y' and islocked='N'")) {
                    ...
                    try (var row = statement.executeQuery()) {
                        ...
                        if (row.next()) throw new OBSecurityException("Ambiguous user identity");
                    }
                }
                String sql = "select r.ad_role_id,r.ad_client_id,ro.ad_org_id "
                        ...
                        + "join ad_org o on o.ad_org_id=ro.ad_org_id "
                        + "where u.ad_user_id=? and ur.isactive='Y' and r.isactive='Y' and ro.isactive='Y' and o.isactive='Y' "
                        + "and (o.ad_client_id=r.ad_client_id or o.ad_org_id='0') "
                        + "and (? is null or r.ad_role_id=?) and (? is null or ro.ad_org_id=?) "
                        + "order by case when r.ad_role_id=u.default_ad_role_id then 0 else 1 end, "
                        + "case when r.ad_client_id='0' then 1 else 0 end, r.ad_role_id, "
                        + "case when ro.ad_org_id=u.default_ad_org_id then 0 else 1 end, "
                        + "case when ro.ad_org_id='0' then 1 else 0 end,ro.ad_org_id limit 1";
                try (var statement = connection.prepareStatement(sql)) {
                    statement.setString(1, identity[0]);
                    statement.setString(2, requestedRole); statement.setString(3, requestedRole);
                    statement.setString(4, requestedOrganization); statement.setString(5, requestedOrganization);
                    try (var row = statement.executeQuery()) {
                        if (!row.next()) throw new OBSecurityException("Role or organization is not authorized");
                        identity[1] = row.getString(1); identity[2] = row.getString(2); identity[3] = row.getString(3);
                    }
                }
            });
        }
